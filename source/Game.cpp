...
EWRAM_LOCATION ALIGN(4) u32 Game::fadeType = 0;
...
void Game::Update()
{
	...
	else if (doFade2)
	{
		...
		else
		{
			...
			targetPalette = NULL;
			GameModeManager::GetScreen()->OnExitCallback();
		}
	}
}
...
void Game::FadeToGreyScale(u32 FrameCount)
{
	if (!doFade)
	{
...
void Game::FadeToPalette(const u16 *newPalette, u32 FrameCount)
{
	if (!doFade)
	{
		currentPalette = GetCurrentPalette();

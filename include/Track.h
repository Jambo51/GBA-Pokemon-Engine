...
private:
	virtual void ExecuteModifications(u8 commandID, u16 tempo, GBPChannel &masterChannel) = 0;
	virtual void ModulateTrack(GBPChannel &masterChannel) = 0;
	virtual u16 GetModulationPitch(GBPChannel &masterChannel) = 0;
	virtual void ArpeggiateTrack(GBPChannel &masterChannel) = 0;
	virtual u16 CalculatePitch(u8 commandID, s8 keyShift, u8 octave, u16 tone) = 0;
	virtual u8 ExecuteCommands(u8 commandID, GBPChannel &masterChannel) = 0;
public:
	Track();
	virtual ~Track();
	virtual void Update(u16 tempo, GBPChannel &masterChannel) = 0;
	virtual void Clear();

...
void ToneTrack::ExecuteModifications(u8 commandID, u16 tempo, GBPChannel &masterChannel)
{
...
u8 ToneTrack::ExecuteCommands(u8 commandID, GBPChannel &masterChannel)
{
	u16* tone1Controller = masterChannel.GetBufferAddress();
	u8 commandLength = 1;
...
void ToneTrack::ModulateTrack(GBPChannel &masterChannel)
{
	u16* tone1Controller = masterChannel.GetBufferAddress();
...
void ToneTrack::ResetModulationArpeggiationCounters(GBPChannel &masterChannel)
{
...
void ToneTrack::ArpeggiateTrack(GBPChannel &masterChannel)
{
	u16* tone1Controller = masterChannel.GetBufferAddress();
...
void ToneTrack::Update(u16 tempo, GBPChannel &masterChannel)
{
	if (noteLength1 < 2)
	{
...
u16 ToneTrack::GetModulationPitch(GBPChannel &masterChannel)
{
	modulationSpeedDelay = modulationSpeed;
	u8 theValue = 0;
	u16 newPitch = pitch;

...
u32 CreateWindow(ScriptRunner* runner)
{
...
u32 DeleteWindow(ScriptRunner* runner)
{
	u8* script = runner->GetScriptPointer();
	u32 windowID = *(script + 1);
...
u32 SetWindowPosition(ScriptRunner* runner)
{
...
u32 SetWindowLayerEnabled(ScriptRunner* runner)
{
	u8* script = runner->GetScriptPointer();
...
u32 SetWindowLayerDisabled(ScriptRunner* runner)
{
	u8* script = runner->GetScriptPointer();
...
{
	ScriptRunner* runner = (ScriptRunner*)data;
	runner->SetWaitFrames(runner->GetWaitFrames() & (~(0x10)));
}

u32 FlashPaletteToWhiteFiftyPercent(ScriptRunner* runner)
{
...
u32 FlashPaletteToBlackFiftyPercent(ScriptRunner* runner)
{
...
u32 WaitPaletteFlash(ScriptRunner* runner)
{
...
u32 CallStandardWindowEffect(ScriptRunner* runner)
{
	u8* script = runner->GetScriptPointer();
	u32 id = *(script + 1);
...
u32 SetBasicWindowAnimation(ScriptRunner* runner)
{
	u8* script = runner->GetScriptPointer();
...
u32 WaitAllWindowAnimations(ScriptRunner* runner)
{
	for (u32 i = 0; i < 4; i++)
	{
...
u32 LoadTiles(ScriptRunner* runner)
{
	u8* script = runner->GetScriptPointer();
...
u32 LoadTilemap(ScriptRunner* runner)
{
	u8* script = runner->GetScriptPointer();
	void* pointer = (void*)UnalignedNumberHandler::LoadUnalignedNumber(script, 1, 4);
...
u32 LoadCompressedTiles(ScriptRunner* runner)
{
	u8* script = runner->GetScriptPointer();
	void* pointer = (void*)UnalignedNumberHandler::LoadUnalignedNumber(script, 1, 4);
...
u32 LoadCompressedTilemap(ScriptRunner* runner)
{
	u8* script = runner->GetScriptPointer();
	void* pointer = (void*)UnalignedNumberHandler::LoadUnalignedNumber(script, 1, 4);
	void* targetPointer = (void*)Game::GetLayerPointer(0);
